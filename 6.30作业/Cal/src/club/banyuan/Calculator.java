  ...
  public static int compute(String[] tokens) throws CalculatorException {
    ...
    switch (tokens.length) {
      ...
      case 2:
        // 只有一种情况，用户输入 负数
        // TODO: complete the cases
        if ("-".equals(tokens[0]) && isInteger(tokens[1])) {
          int number = Integer.parseInt(tokens[1]);
          return -number;
        } else if ("-".equals(tokens[0]) && !isInteger(tokens[1])) {
          throw new IllegalInputException("Illegal Argument");
        } else {
...
//        boolean isInteger(tokens[0]) = isInteger(tokens[0]);
//        boolean isInteger(tokens[2]) = isInteger(tokens[2]);
...
//        int num2 = Integer.parseInt(tokens[2]);
        ...
        if (isInteger(tokens[0]) && !isOperator(tokens[1]) && isInteger(tokens[2])) {
          throw new IllegalInputException("Illegal Operator");
        }
