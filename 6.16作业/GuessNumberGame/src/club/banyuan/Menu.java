...
public class Menu {
  ...
  private static final int[] guessNumber = new int[COUNT];
  private static int inputGuessTime;
  private static int residueNumber;
  private static int needGuessNumber;
  ...
  public static void gameLoad() {
    ...
    for (int i = 0; i < inputGuessTime - residueNumber; i++) {
      if (guessNumber[i] == needGuessNumber) {
        ...
        System.out.println("第" + (i + 1) + "次" + " " + guessNumber[i] + " => " + (guessNumber[i]
            - needGuessNumber));
      }
    }
  }
